...
def objective(trial):
    ...
    def trainloop_IF(train_loader, val_loader, model, optimizer, criterion, device, num_epochs):
    ...
    def trainloop_IFOF(train_loader, val_loader, model, optimizer, criterion, device, num_epochs):
        best_val_loss = float('inf')
        no_improvement = 0
        best_model = None
        history = {'train_loss': [], 'val_loss': [], 'train_accuracy': [], 'val_accuracy': [],
...
with open('hyper/optuna_study_freeze.pkl', 'wb') as f:
    pickle.dump(study, f)
...
torch.save(best_model_ResNet_IF.state_dict(), 'hyper/best_Fmodel_ResNet_IF.pth')
torch.save(best_model_ResNet_IFOF.state_dict(), 'hyper/best_Fmodel_ResNet_IFOF.pth')
torch.save(best_model_EfficientNet_IF.state_dict(), 'hyper/best_Fmodel_EfficientNet_IF.pth')
torch.save(best_model_EfficientNet_IFOF.state_dict(), 'hyper/best_Fmodel_EfficientNet_IFOF.pth')

# Save each best history to a .pkl file
with open('hyper/best_Fhistory_ResNet_IF.pkl', 'wb') as f:
    pickle.dump(best_history_ResNet_IF, f)

with open('hyper/best_Fhistory_ResNet_IFOF.pkl', 'wb') as f:
    pickle.dump(best_history_ResNet_IFOF, f)

with open('hyper/best_Fhistory_EfficientNet_IF.pkl', 'wb') as f:
    pickle.dump(best_history_EfficientNet_IF, f)

with open('hyper/best_Fhistory_EfficientNet_IFOF.pkl', 'wb') as f:
