...
class WACVDatasetAU(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(features_df) > 1:
            features_row = features_df.loc[features_df.iloc[:, 1].idxmax()]
        else:
            features_row = features_df.iloc[0]

        au_features = features_row.loc['AU01_r':'AU45_c'].values
        au_features = StandardScaler().fit_transform(au_features.reshape(-1, 1)).flatten()
        au_features = torch.tensor(au_features, dtype=torch.float32)

        return image, au_features, torch.tensor(label, dtype=torch.long)
...
def get_dataloaders_au(image_dir, feature_dir, batch_size=32, val_split=0.2, test_split=0.1):
    ...
    dataset = WACVDatasetAU(image_dir, feature_dir, transform=transform)
